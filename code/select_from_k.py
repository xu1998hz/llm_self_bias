...
name_dict = {'vicuna': 'lmsys/vicuna-7b-v1.5', 'llama': 'yahma/llama-7b-hf', 'llama2-7b': 'meta-llama/Llama-2-7b-chat-hf',\
             ...
             "llama2-chat-ift-mqm": "/home/guangleizhu/peril_self_improve/instruct_ft/ckpt/llama2_ift_mqm/checkpoint-374", 'mistral-inst1': 'mistralai/Mistral-7B-Instruct-v0.1', \
...
@click.option("-final_out_name")
@click.option("-final_score_name")
@click.option(
...
def main(lang_dir, savename, base_name, final_out_name, final_score_name, api_source, model_type, task_type, batch_size, k, instructscore_enable):
    print('instruction score enabled:', instructscore_enable)
    ...
    src_lines = [ele for ele in src_lines for _ in range(k)]
    # for i in src_lines:
    #     print(i)
    print(len(src_lines))
    print(len(out_lines))
    out_lines = out_lines[:len(src_lines)]
        ...
        for index, (src_batch_txt, src_batch_out) in enumerate(zip(batchify(src_lines, batch_size), batchify(out_lines, batch_size))):
            if task_type == "mt":
                if model_type == 'llama2-chat-ift-mqm':
                    ...
                    # for src_txt, out in zip(src_batch_txt, src_batch_out):
                    #     print(f"""Source: ```{src_txt[:-1]}``` Translation: ```{out[:-1]}``` Annotate errors in the translation. MQM annotations:""")
            elif task_type == "commonsenseQA":
            ...
            if instructscore_enable:
                ...
                if api_source == "openai":
                    ...
                    #     response = eval_lines[index]
                        ...
                        if model_type == "llama2-chat-ift-mqm":
                            inputs = tokenizer([prompt_txt for prompt_txt in prompt_txt_ls], return_tensors="pt", padding=True, truncation=True, max_length=720).to(model.device)
                        else:
                        ...
                        if task_type == "mt":
                            ...
                            elif model_type == "llama2-chat-ift-mqm":
                                response_ls = [response.replace(prompt_txt, "").strip() for prompt_txt, response in zip(prompt_txt_ls, response_ls)]
                            else:
                                response_ls = [response.split("Annotate errors in the translation. MQM annotations:")[4].split("\n\n")[0].strip() for prompt_txt, response in zip(prompt_txt_ls, response_ls)]
                                # print(len(response_ls))
                                response_ls = [response.replace(prompt_txt, "") for prompt_txt, response in zip(prompt_txt_ls, response_ls)]
                        else:
    ...
    for i in range(n):
        ...
        print('best:', best)
        selected_idx = best[0]
        final_score.append(out_ls[selected_idx])
        final_out.append(out_lines[selected_idx])
    with open(final_out_name, "w") as f:
        f.writelines(final_out)
        print(f"{final_out_name} is saved!")
    with open(final_score_name, "w") as f:
        f.writelines(final_score)
        print(f"{final_score_name} is saved!")
