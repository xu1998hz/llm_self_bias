# sh code/iterative_refine.sh -m mistral-inst2-mqm -a transformers -l yor-en -d 6 -b 64 
while getopts "m:a:l:d:t:b:s:i:" flag
do
    case "${flag}" in
        m) model_name=${OPTARG};;
        a) api_source=${OPTARG};;
...
OPENAI_API_KEY="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" CUDA_VISIBLE_DEVICES="${device_id}" python3 code/generate_feedback.py -task_type mt -lang_dir "${lang}" \
    ...
    -model_type "${model_name}" -savename "model_outputs/${model_name}/self_refine/${lang}/${model_name}-scores/${lang}_eval_100_one-shot_${model_name}_new_0_rerun.txt"  -batch_size "${batch_size}"
...
OPENAI_API_KEY="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" CUDA_VISIBLE_DEVICES="${device_id}" python3 code/refinement.py -lang_dir "${lang}" -task_type mt \
    -start_index "${init_step}" -iteration "${step}" -api_source "${api_source}" -model_type "${model_name}"