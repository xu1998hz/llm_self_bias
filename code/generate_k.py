...
def main(lang_dir, api_source, model_type, task_type, save_name, batch_size, k):
    print('k = ', k)
    ...
    with tqdm(total=len(src_lines)) as pbar:
        for batch_line in batchify(src_lines, batch_size):
            ...
            if api_source == "openai":
                ...
                if task_type == "mt":
                    if model_type == "mistral_moe":
                        responses = [response.replace(prompt_txt, '').split('\n\n')[0].strip() for response, prompt_txt in zip(responses, prompt_txt_ls * k)]
                    else:
                        # for r in responses:
                        #     tmp = r.split('### Instruction:')[4].split(f"### {tgt_lang}:")[1].split("\n\n")[0].strip()
                        #     print(tmp)
            ...
            pbar.update(int(len(responses) / k))

    with open(save_name, "wb") as f:
        f.writelines([s.encode('utf-8') for s in out_ls])
